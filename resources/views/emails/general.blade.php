                ...
                <td style="padding: 30px 30px 20px;" >
                    ...
                    @foreach ($details as $item)
                        ...
                        @elseif (isset($item['type']) && $item['type'] === 'action')
                            <a href="{{$item['value']['link']}}" style="background-color: #9769ff;border-radius:4px;color:#ffffff;display:inline-block;font-size:13px;font-weight:600;line-height:44px; margin-bottom: 25px; text-align:center;text-decoration:none;text-transform: uppercase; padding: 0 25px">{{$item['value']['action']}}</a>
                        @elseif (isset($item['type']) && $item['type'] === 'code')
                            <h1 style="padding: 10px 40px; border-radius: 6px; margin-bottom: 20px; text-align: center; background: #6a29ff;"><b>{{$item['value']}}</b></h1>
                        @endif
                    @endforeach
